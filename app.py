...
with st.container(border=True):
    ...
    if protokoll_typ == "Zählerübergabeprotokoll":
        ...
        st.write("**Einbehalt der Kaution**")
        ...
        with col_e2:
            kaution_einbehalt = st.text_input(
                ...
                placeholder="z.B. offene Nachzahlungen...",
...
with st.container(border=True):
    ...
    if "zaehler_liste" not in st.session_state:
        st.session_state.zaehler_liste = [
            ...
            {"typ": "Wasser", "bezeichnung": "Wasser Kalt", "einheit": "m³"},
            {"typ": "Wasser", "bezeichnung": "Wasser Warm", "einheit": "m³"},
            {"typ": "Heizung", "bezeichnung": "Heizung", "einheit": "Einheiten"},
        ]

    with st.expander("Zähler hinzufügen"):
        z_typ = st.selectbox(
            "Zählertyp",
            ["Strom", "Wasser", "Heizung", "Gas", "Sonstige"],
            key="select_z_typ",
        )
        z_bez = st.text_input("Bezeichnung (z.B. Küche, Bad, Keller)", key="neu_zaehler_bez")
        z_einheit = st.text_input(
            "Maßeinheit (z.B. kWh, m³, Einheiten)", value="kWh", key="neu_zaehler_einheit"
        )
    ...
    if not st.session_state.zaehler_liste:
        st.info("Aktuell sind keine Zähler eingetragen. Füge oben einen Zähler hinzu.")
    
    for i, z in enumerate(list(st.session_state.zaehler_liste)):
        col_head, col_del = st.columns([5, 1])
        with col_head:
            st.markdown(f"**{z['typ']}** – {z['bezeichnung']}")
        with col_del:
            if st.button("Löschen", key=f"del_z_{i}", help="Zähler löschen"):
...
if st.button(
    ...
    if not wohnung or not mieter:
        ...
        for z in zaehler_daten:
            ...
            pdf.set_font("helvetica", size=10)
            pdf.cell(70, 6, f"{z['bezeichnung']} (Nr: {z['nummer'] if z['nummer'] else '-'})", 0, 0)
            pdf.set_font("helvetica", "B", 10)
            pdf.cell(
                0,
                6,
